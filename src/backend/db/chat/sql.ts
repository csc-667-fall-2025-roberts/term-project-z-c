...
// Waiting room chat - create message
export const CREATE_WAITING_ROOM_MESSAGE = `
WITH user_info AS (
  SELECT id, COALESCE(display_name, username) AS display_name
  FROM users
  WHERE id = $1
...
// Waiting room chat - recent messages
export const RECENT_WAITING_ROOM_MESSAGES = `
SELECT
  chat.id,
  chat.user_id,
  chat.message,
...
FROM chat
JOIN users ON users.id = chat.user_id
WHERE chat.scope = 'waiting_room' AND chat.game_id = $1
ORDER BY chat.time_sent ASC
`;
